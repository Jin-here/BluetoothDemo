...
import static com.vgaw.bluetoothdemo.page.bluetooth.BluetoothActivity.MSG_RECEIVED;
import static com.vgaw.bluetoothdemo.page.bluetooth.BluetoothActivity.MSG_SENT;
        ...
        while (!isStop.get()){
            switch (behaviour){
                case BEHAVIOUR_RECEIVE:
                    try {
                        msg = reader.readLine();
                        handler.sendMessage(handler.obtainMessage(MSG_RECEIVED, msg));
                        Log.e("fuck", msg);
    ...
    private void sendMsgInternal(){
        if (writer != null){
            try {
                writer.write(msg);
                writer.flush();
                handler.sendMessage(handler.obtainMessage(MSG_SENT, msg));
                Log.e("fuck", "消息发送成功");
            } catch (IOException e) {
                cancel();
            }
