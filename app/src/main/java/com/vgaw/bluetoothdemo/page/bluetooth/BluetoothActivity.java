...
public class BluetoothActivity extends AppCompatActivity {
    ...
    public static final int MSG_CONNECTED = 0x80;
    public static final int MSG_ACCEPTED = 0x81;
    public static final int MSG_RECEIVED = 0x82;
    public static final int MSG_SENT = 0x83;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (mBluetoothAdapter == null) {
            Toast.makeText(BluetoothActivity.this, "设备不支持Bluetooth", Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
        makeChoice();
            ...
            switch (v.getId()) {
                ...
                case R.id.btn_search:
                    ...
                    if (!mBluetoothAdapter.startDiscovery()) {
                        Toast.makeText(BluetoothActivity.this, "开始搜索出现异常", Toast.LENGTH_SHORT).show();
                    } else {
                        deviceListFragment.clearDevice();
                        Toast.makeText(BluetoothActivity.this, "正在进行搜索", Toast.LENGTH_SHORT).show();
                    }
                    break;
                // 客户端：停止搜索
                case R.id.btn_cancel:
                    if (mBluetoothAdapter.isDiscovering()) {
                        if (mBluetoothAdapter.cancelDiscovery()) {
                            Toast.makeText(BluetoothActivity.this, "已经停止", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(BluetoothActivity.this, "停止出现异常", Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                // 客户端：发送消息
